...
use js_instrumentation_shared::transform_options::{TransformOptions, TransformStrategy};
...
pub fn transform(filename: &str, code: &str, options: JsValue) -> Result<JsValue, JsError> {
    ...
    let transform_output = match options.transform_strategy {
        Some(TransformStrategy::AST) | None => {
            js_instrumentation_transform::apply_transform(filename, code, &options)
        }
    };
